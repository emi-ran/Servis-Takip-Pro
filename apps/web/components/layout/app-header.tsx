...
import type { ShellRoleKey } from "@/lib/api/shell";
...
export function AppHeader({ dictionary, shellContext, onOpenSidebar }: AppHeaderProps) {
  const roleKey = shellContext.currentUser.roleKey as ShellRoleKey;
    ...
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur md:px-8">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          <p className="hidden truncate text-sm font-medium text-slate-500 sm:block">{dictionary.header.welcome}</p>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="hidden text-right sm:block">
          <p className="text-sm font-semibold text-slate-900">{shellContext.currentUser.name}</p>
          <p className="text-xs text-slate-500">{dictionary.roles[roleKey]}</p>
        </div>
        <div className="flex h-8 w-8 items-center justify-center rounded-full border border-blue-200 bg-blue-100 text-sm font-bold text-blue-600 shadow-sm md:h-10 md:w-10">
          {shellContext.currentUser.initials}
        </div>
      </div>
    </header>
  );
