...
export default async function QuickScanPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const resolvedLocale: Locale = isLocale(locale) ? locale : "tr";
  const dictionary = await getDictionary(resolvedLocale);

  return <ComingSoonView locale={resolvedLocale} dictionary={dictionary} sectionTitle={dictionary.navigation.items.quickScan} />;
}
