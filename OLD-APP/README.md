...
- `/[locale]/parts` placeholder kaldırıldı; Stok & Parça overview ekranı aktif hale getirildi. Özet kartları, arama + durum/kategori filtreleri, parça uyumluluk/stok görünümü, son stok hareketleri ve düşük stok uyarı paneli eklendi.
- Parça rezervasyon akışı yalnızca istemci tarafı mock davranıştır: seçili parça + servis kaydı için non-persistent rezervasyon ve hareket satırı üretilir, görünürdeki ayrılmış adet güncellenir, ancak gerçek stock movement, audit log, tenant enforcement ve RBAC henüz uygulanmış sayılmaz.
- Parça modülünde satış değeri özeti ve maliyet görünürlüğü permission-sensitive mock olarak işaretlendi; gerçek fiyat/maliyet, tenant scope ve rol bazlı backend enforcement olmadan güvenli kabul edilmemelidir.
- `/[locale]/reports` placeholder yerine Reports overview ekranı eklendi; tarih aralığı mock filtreleri (bugün, bu hafta, bu ay, son 30 gün), operasyon özet kartları, servis durum dağılımı, kategori/cihaz/servis tipi kırılımı ve export readiness paneli aynı görünümde sunuluyor.
- Reports ekranındaki finans özetleri ile personel performans alanları açıkça permission-sensitive/mock olarak işaretlendi; bu ekran gerçek yetki enforcement, tenant scope veya kalıcılık sağlıyormuş gibi değerlendirilmemelidir.
- Reports ekranındaki “Raporu / export'u hazırla” aksiyonu gerçek dosya üretmez; yalnızca istemci tarafında non-persistent readiness mesajı gösterir.
- Gerçek raporlama için backend aggregation, tenant filtering, server-side RBAC ve export/audit logging zorunludur; bu gereksinimler README, CHANGELOG ve UI caveat metinlerinde açıkça belirtilmiştir.
...
- `/[locale]/settings` placeholder kaldırıldı; Genel Ayarlar ekranı aktif hale getirildi. Firma profili alanları (firma adı, vergi numarası, telefon, e-posta, varsayılan dil/para birimi/saat dilimi) yalnızca istemci tarafında mock düzenlenebilir.
- Ayarlar ekranındaki şube/operasyon özeti, bildirim readiness toggle'ları ve güvenlik checklist'i planlama amaçlıdır; backend kalıcılığı, tenant config write, bildirim teslimatı, audit log ve gerçek auth/RBAC/tenant enforcement henüz uygulanmamıştır.
