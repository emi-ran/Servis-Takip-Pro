    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {submitError ? (
        <Panel className="border-red-200 bg-red-50 p-4">
          <p className="text-sm font-semibold text-red-800">{dictionary.serviceRecords.newForm.error.title}</p>
          <p className="mt-1 text-sm text-red-700">{submitError}</p>
          <button
            className="mt-2 inline-flex items-center justify-center rounded-lg border border-red-300 px-3 py-1.5 text-xs font-medium text-red-700 transition hover:bg-red-100"
            onClick={() => setSubmitError(null)}
            type="button"
          >
            {dictionary.serviceRecords.newForm.error.dismiss}
          </button>
        </Panel>
      ) : null}
